package com.codeclan.example.jurassic.components;

import com.codeclan.example.jurassic.models.*;
import com.codeclan.example.jurassic.repositories.dinosaurs.DinosaurRepository;
import com.codeclan.example.jurassic.repositories.paddocks.PaddockRepository;
import com.codeclan.example.jurassic.repositories.species.SpeciesRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.ApplicationArguments;
import org.springframework.boot.ApplicationRunner;
import org.springframework.stereotype.Component;

@Component
public class DataLoader implements ApplicationRunner {

    @Autowired
    DinosaurRepository dinosaurRepository;

    @Autowired
    PaddockRepository paddockRepository;

    @Autowired
    SpeciesRepository speciesRepository;

    public DataLoader(){

    }

    public void run(ApplicationArguments args){

        Paddock catrinCanyon = new Paddock(4, "Catrin Canyon", true, false, "./paddock_01.svg");
        paddockRepository.save(catrinCanyon);

        Paddock morgaineMeadows = new Paddock(5, "Morgaine Meadows", true, false, "./paddock_02.svg");
        paddockRepository.save(morgaineMeadows);

        Paddock nelsonNubbin = new Paddock(4, "Nelson's Nubbin", true, true, "./paddock_03.svg");
        paddockRepository.save(nelsonNubbin);

        Paddock eduardoEnclave = new Paddock(3, "Eduardo Enclave", true, true, "./paddock_04.svg");
        paddockRepository.save(eduardoEnclave);

        Paddock goldblumGorge = new Paddock(6, "Goldblum Gorge", false, true, "./paddock_05.svg");
        paddockRepository.save(goldblumGorge);

        Paddock sandySandpit = new Paddock(2, "Sandy's Sandpit", false, true, "./paddock_06.svg");
        paddockRepository.save(sandySandpit);

        Paddock johnJungle = new Paddock(5, "John's Jungle", false, true, "./paddock_07.svg");
        paddockRepository.save(johnJungle);

        Species brontosaurus = new Species("Brontosaurus", "Herbivore", 10,"./DinosaurImages/Artboard_2.svg");
        speciesRepository.save(brontosaurus);

        Species tsintaosaurus = new Species("Tsintaosaurus", "Herbivore",6, "./DinosaurImages/Artboard_4.svg");
        speciesRepository.save(tsintaosaurus);

        Species stegosaurus = new Species("Stegosaurus", "Herbivore",7, "./DinosaurImages/Artboard_5.svg");
        speciesRepository.save(stegosaurus);

        Species parasaurolophus = new Species("Parasaurolophus", "Herbivore",5, "./DinosaurImages/Artboard_6.svg");
        speciesRepository.save(parasaurolophus);

        Species triceratops = new Species("Triceratops", "Herbivore",8, "./DinosaurImages/Artboard_7.svg");
        speciesRepository.save(triceratops);

        Species tyrannosaurus = new Species("Tyrannosaurus", "Carnivore",12, "./DinosaurImages/Artboard_8.svg");
        speciesRepository.save(tyrannosaurus);

        Species talarurus = new Species("Talarurus", "Herbivore",8, "./DinosaurImages/Artboard_10.svg");
        speciesRepository.save(talarurus);

        Species coelophysis = new Species("Coelophysis", "Carnivore",5, "./DinosaurImages/Artboard_9.svg");
        speciesRepository.save(coelophysis);

        Species diplodocus = new Species("Diplodocus", "Herbivore", 10, "./DinosaurImages/Artboard_16.svg");
        speciesRepository.save(diplodocus);

        Species velociraptor = new Species("Velociraptor", "Carnivore", 6, "./DinosaurImages/Artboard_15.svg");
        speciesRepository.save(velociraptor);

        Dinosaur juan = new Dinosaur("Juan", 'M', diplodocus, eduardoEnclave);
        dinosaurRepository.save(juan);

        Dinosaur sky = new Dinosaur("Sky", 'M', stegosaurus, nelsonNubbin);
        dinosaurRepository.save(sky);

        Dinosaur colin = new Dinosaur("Colin", 'M', triceratops, nelsonNubbin);
        dinosaurRepository.save(colin);

        Dinosaur sophia = new Dinosaur("Sophia", 'F', velociraptor, morgaineMeadows);
        dinosaurRepository.save(sophia);

        Dinosaur kat = new Dinosaur("Kat", 'F', tyrannosaurus, catrinCanyon);
        dinosaurRepository.save(kat);

        Dinosaur eugene = new Dinosaur("Eugene", 'M', talarurus, eduardoEnclave);
        dinosaurRepository.save(eugene);




    }


}
